...
from .serializers import Xls_Serializer
...
class XlsViewSet(viewsets.ModelViewSet):
    queryset = Xls_Model.objects.all()
    ...
    def create(self, request, *args, **kwargs):                    
        ...
        if file_name.endswith('.xls'):
            ...
            with open('xls_tojson.json',"w") as arq_json:
                arq_json.write("")
                arq_json.close()

                df = pd.read_excel("media/xls/" + file_name, sheet_name=sheet_name, header=header, 
                ...
                f = df.to_json('xls_tojson.json', indent=1, orient='records')

            
                return Response(serializer.data)
            return Response('Error Parameter or Extension File is Wrong ')
                    
    @action(detail=True, methods=['GET'])
    def my_post(self,request, pk=None):
        queryset = Xls_Model.objects.latest('id')
        
        if self.create is not None:            
            with open('xls_tojson.json') as m_json:
