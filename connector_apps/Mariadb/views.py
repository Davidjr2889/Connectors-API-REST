...
from .serializers import MariaDb_Serializer
...
class MariaDbViewSet(viewsets.ModelViewSet):

    queryset = MariaDb_Model.objects.all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        table = serializer.data['table_mariadb']
        ...
        with open(f'Mariadb{pk}.json','w') as m_json:
            m_json.write("")
            m_json.close()
            
            sent_file = find_all().to_json(f'Mariadb{pk}.json', indent=1, orient='records')
    ...
    def my_post(self,request,pk=None):
        with open(f'Mariadb{pk}.json') as m_json:
    ...
    def comunicator(self,request, pk=None):
        ...
        queryset = MariaDb_Model.objects.latest('id')
